...
describe '/api/v1/products', :api do
  ...
  describe '#index' do
    before { get api_v1_products_path, access_token: token.token }
  ...
  describe '#show' do
    ...
    it { get api_v1_product_path(product), access_token: token.token
         last_response.status.should eq 200 }

    it 'returns the id, name, price and location' do
      get api_v1_product_path(product), access_token: token.token
    ...
    it 'returns a 404 if the product does not exist' do 
      get api_v1_product_path id: 20, access_token: token.token
      last_response.status.should eq 404 
    end
  end

  describe '#create' do
    it 'returns a 401 without an access_token token' do 
    ...
    it 'returns a 400 if the product has no name' do 
      post api_v1_products_path product: { net_price_pennies: 100, net_price_currency: 'GBP' }, access_token: token.token
      last_response.status.should eq 400
    end

    it 'returns a 400 if the product has no net_price' do 
      post api_v1_products_path product: { name: 'Invalid Product' }, access_token: token.token
      last_response.status.should eq 400
    end

    it 'returns a 200 if the product is valid' do
      post api_v1_products_path product: { name: 'Valid Product', net_price_pennies: 100, net_price_currency: 'GBP' }, access_token: token.token
      last_response.status.should eq 200
    end
  end
  ...
  describe '#update' do
    ...
    it 'returns a 200 if the product is valid' do
      put api_v1_product_path product: { name: 'Valid Product', net_price_pennies: 100 }, id: product.id, access_token: token.token
      last_response.status.should eq 200
    end

    it 'returns a 400 if the net_price is 0' do 
      put api_v1_product_path product: { name: 'Valid Product', net_price_pennies: 0 }, id: product.id, access_token: token.token
      last_response.status.should eq 400
    end    

    it 'returns a 404 if the product does not exist' do
      put api_v1_product_path product: { name: 'Valid Product', net_price_pennies: 100 }, id: 0, access_token: token.token
      last_response.status.should eq 404
    end
  end
  ...
  describe '#destroy' do
    ...
    it 'returns a 200 if the product was never used in an order' do
      delete api_v1_product_path id: product.id, access_token: token.token
      last_response.status.should eq 200
    end
    ...
    it 'returns a 404 if the product does not exist' do
      delete api_v1_product_path id: 0, access_token: token.token
      last_response.status.should eq 404
    end
  end
end