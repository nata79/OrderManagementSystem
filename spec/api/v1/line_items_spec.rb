...
describe '/api/v1/orders/:id/line_items', :api do
  ...
  describe '#index' do
    before { get api_v1_order_line_items_path order }
  ...
  describe '#show' do
    ...
    it { get api_v1_order_line_item_path(order, line_item); last_response.status.should eq 200 }

    it 'returns the id, product, quantity, net_price and location' do      
      get api_v1_order_line_item_path(order, line_item)
  ...
  describe '#create' do
    ...
    it 'returns a 400 if quantity is 0' do
      post api_v1_order_line_items_path order_id: order.id, line_item: { product_id: create(:product).id, quantity: 0 }
      last_response.status.should eq 400
    end

    it 'returns a 400 if no product is passed' do
      post api_v1_order_line_items_path order_id: order.id, line_item: { quantity: 0 }
      last_response.status.should eq 400
    end

    context 'order status' do
      it 'returns a 200 if order is draft' do
        post api_v1_order_line_items_path order_id: order.id, line_item: { product_id: create(:product).id, quantity: 10 }
  ...
  describe '#update' do
    ...
    it 'returns a 400 if quantity is 0' do
      put api_v1_order_line_item_path order_id: order.id, id: line_item.id, line_item: { product_id: create(:product).id, quantity: 0 }
      last_response.status.should eq 400
    end
    
    context 'order status' do
      it 'returns a 200 if order is draft' do
        put api_v1_order_line_item_path order_id: order.id, id: line_item.id, line_item: { product_id: create(:product).id, quantity: 15 }
        last_response.status.should eq 200
      end
